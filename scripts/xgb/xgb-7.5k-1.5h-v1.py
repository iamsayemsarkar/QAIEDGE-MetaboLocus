...
# VERSION:  4.0 (Precision Edition for 7.5k Metabolites)
# PURPOSE:
#   To achieve the absolute theoretical peak predictive performance on a focused,
#   high-stakes dataset of 7,500 metabolites. This script is a ground-up
#   redesign, with every parameter and strategy recalibrated for precision,
#   robust generalization, and the prevention of overfitting.
#
# KEY FEATURES:
#   1. PRECISION FEATURE ENGINEERING: Utilizes a curated "Precision Set" of 18
#      high-impact descriptors and a 1024-bit fingerprint, maximizing the
#      signal-to-noise ratio for this dataset size.
#   2. FOCUSED DEEP OPTIMIZATION: Implements a 90-minute Optuna deep search
#      with a constrained search space to guide the model towards solutions of
#      optimal complexity and robustness.
#   3. UNIMPEACHABLE REPRODUCIBILITY: Saves the final model and all versioned
#      artifacts, including the exact data splits, to a portable archive.
# ==============================================================================
...
# ==============================================================================
# STEP 2: IMPORTS & UNRIVALED CONFIGURATION FOR 7.5K
# ==============================================================================
...
# --- Global Constants: Calibrated for 7,500 Metabolites ---

DRIVE_XML_FILE_PATH: str = '/content/drive/My Drive/hmdb/hmdb_metabolites.xml'
MAX_METABOLITES_TO_PARSE: int = 7500
OUTPUT_ARCHIVE_NAME: str = 'XGB_Unrivaled_Run_7.5k.zip'
...
# --- Optuna Configuration: 90-Minute Focused Deep Search ---

OPTUNA_TIMEOUT_SECONDS: int = 5400  # 90 minutes
OPTUNA_EARLY_STOPPING_ROUNDS: int = 40

# --- Feature Engineering Configuration: The Precision Set ---

COMPUTED_NUMERICAL_FEATURES: List[str] = [
    # Core Properties (9)
    'MolWt', 'HeavyAtomCount', 'TPSA', 'NumHAcceptors', 'NumHDonors',
    'MolLogP', 'NumRotatableBonds', 'RingCount', 'FractionCSP3',
    # Electronic & Shape Properties (4)
    'qed', 'HallKierAlpha', 'MaxPartialCharge', 'MinPartialCharge',
    # Key Functional Groups (5)
    'fr_NH2', 'fr_COO', 'fr_phenol', 'fr_ketone', 'fr_ether'
]
...
def compute_features_from_smiles(smiles: str) -> Optional[Dict[str, Any]]:
    """
    Computes a 1024-bit Morgan fingerprint and the 18 "Precision Set" descriptors.
    """
    try:
        ...
        descriptors = {
            ...
            'MinPartialCharge': Descriptors.MinPartialCharge(mol), 'fr_NH2': Fragments.fr_NH2(mol),
            'fr_COO': Fragments.fr_COO(mol), 'fr_phenol': Fragments.fr_phenol(mol),
...
def objective(trial, X_train, y_train, X_val, y_val):
    """
    Defines the Optuna objective function with a search space strategically
    constrained to favor robust, generalizable models.
    """
    param_grid = {
...
if __name__ == "__main__":
    print("--- Starting Unrivaled XGBoost Execution Workflow for 7.5k Metabolites ---")
    ...
    if not os.path.exists(DRIVE_XML_FILE_PATH):
        ...
        if not parsed_data:
            ...
            if X is not None:
                # 70/15/15 Data Split with robust stratified fallback
                try:
                ...
                print(f"--- Starting {OPTUNA_TIMEOUT_SECONDS/3600:.2f}-Hour Focused Deep Hyperparameter Search ---")
                ...
                model_filename = 'xgb_unrivaled_model_7.5k.pkl'
                ...
                train_df = df_train.copy(); val_df = df_val.copy(); test_df = df_test.copy()
